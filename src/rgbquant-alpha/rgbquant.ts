...
(function(){
	function RgbQuant(opts) {
		...
		this.palLocked = opts.palLocked || false;
		// palette sort order
//		this.sortPal = ['hue-','lum-','sat-'];
	...
	RgbQuant.prototype.sample = function sample(img, width) {
		...
		var data = customgetImageData(img, width);
	...
	RgbQuant.prototype.reduce = function reduce(img, retType, dithKern, dithSerp) {
		...
		if (dithKern)
			...
			var data = customgetImageData(img),
				buf32 = data.buf32,
				len = buf32.length,
...
//			buf8 = data.buf8,
		...
		for (var y = 0; y < height; y++) {
			...
			for (var x = (dir == 1 ? 0 : width - 1), xend = (dir == 1 ? width : 0); x !== xend; x += dir) {
				...
				//Pixel is transparent, don't try to dither
				if(a1 == 0)
				{
				...
				for (var i = (dir == 1 ? 0 : ds.length - 1), end = (dir == 1 ? ds.length : 0); i !== end; i += dir) {
					...
					if (x1 + x >= 0 && x1 + x < width && y1 + y >= 0 && y1 + y < height) {
							...
							var pixelssss = buf32[idx2];
							var r3 = (pixelssss & 0xff),
							g3 = (pixelssss >>> 8) & 0xff,
							b3 = (pixelssss >>> 16) & 0xff,
							a3 = (pixelssss >>> 24) & 0xff;
							if(a3 != 255)
							{
								continue;
							}
...
RgbQuant.prototype.dither3 = function(img, distcalc: AbstractDistanceCalculator, width: number , height:number, kernel, serpentine:boolean) {
			var kernels = {
				FloydSteinberg: [
					[7 / 16, 1, 0],
					[3 / 16, -1, 1],
				...
				//var distanceCalculator = new iq.distance.Manhattan();
					
				var _kernel = kernels[kernel];
				var data = customgetImageData(img, width),
				buf32 = new Uint32Array(data.buf32.buffer);

        var originalPoint: Array<number>  = [],
              errorLines : number[][][] = [];
        ...
        for (let i = 0; i < _kernel.length; i++) {
            const kernelErrorLines = _kernel[ i ][ 2 ] + 1;
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = xStart, idx = lni + xStart; x !== xEnd; x += dir, idx += dir) {
                ...
                const error = errorLine[ x ];
								...
								originalPoint = [r1,g1,b1,a1];

                const correctedPoint = [
                    this.inRange0to255Rounded(r1 + error[ 0 ]),
                    this.inRange0to255Rounded(g1 + error[ 1 ]),
                    this.inRange0to255Rounded(b1 + error[ 2 ]),
                    this.inRange0to255Rounded(a1 + error[ 3 ])
								];

                const palettePoint = this.getNearestColor2(distcalc, correctedPoint, point),
									r2 = (palettePoint & 0xff),
									g2 = (palettePoint >>> 8) & 0xff,
									b2 = (palettePoint >>> 16) & 0xff,
									a2 = (palettePoint >>> 24) & 0xff;
                ...
                if (this._minColorDistance) {
                    //const dist = distcalc.calculateNormalized(point, palettePoint);
                ...
                if (this._calculateErrorLikeGIMP) {
                    er = correctedPoint[0] - r2;
                    eg = correctedPoint[1] - g2;
                    eb = correctedPoint[2] - b2;
                    ea = correctedPoint[3] - a2;
                } else {
                    er = originalPoint[0] - r2;
                    eg = originalPoint[1] - g2;
                    eb = originalPoint[2] - b2;
                    ea = originalPoint[3] - a2;
                }
...
// adapted from http://jsbin.com/iXofIji/2/edit by PAEz
    ...
    RgbQuant.prototype.dither2 = function(img, distcalc: AbstractDistanceCalculator, width: number , height:number, kernel, serpentine:boolean) {
			var kernels = {
				FloydSteinberg: [
					[7 / 16, 1, 0],
					[3 / 16, -1, 1],
    ...
    RgbQuant.prototype.getNearestColor2 = function getNearestColor2(colorDistanceCalculator : AbstractDistanceCalculator, crgba: Array<number>, uint32:number) {
        return this.idxi32[ this.getNearestIndex2(colorDistanceCalculator, crgba, uint32) | 0 ];
    }

		RgbQuant.prototype._nearestPointFromCache2 = function _nearestPointFromCache2(key : string) {
				return typeof this.i32idx[ key ] === "number" ? this.i32idx[ key ] : -1;
		}

		RgbQuant.prototype.getNearestIndex2 = function getNearestIndex2(colorDistanceCalculator : AbstractDistanceCalculator, crgba: Array<number>, uint32:number) : number {
			let idx : number = this._nearestPointFromCache2("" + uint32);
			if (idx >= 0) return idx;

			let minimalDistance : number = Number.MAX_VALUE;

			idx = 0;
			for (let i = 0, l = this.idxrgb.length; i < l; i++) {
					const p        = this.idxrgb[ i ],
								distance = colorDistanceCalculator.calculateRaw(crgba, p);
			...
			this.i32idx[uint32] = idx;
			return idx;
		}


	RgbQuant.prototype.inRange0to255Rounded = function inRange0to255Rounded(n : number) {
			n = Math.round(n);
			if (n > 255) n = 255;
			else if (n < 0) n = 0;
			return n;
	...
	RgbQuant.prototype._fillErrorLine = function _fillErrorLine(errorLine : number[][], width : number) : void {
			// shrink
